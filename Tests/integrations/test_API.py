...
def test_store_valid_cash_set_get(store_setup, args):
    test_key, test_value, result = args
    store_setup.cache_set(test_key, test_value, 60)
    assert store_setup.cache_get(test_key) == result


def test_store_valid_get(store_setup):
    test_key, test_value = 'test', ['test1', 'test2']
    store_setup.cache_set(test_key, test_value, 60)
    assert store_setup.get(test_key) == bytes(str(test_value), encoding='utf-8')
